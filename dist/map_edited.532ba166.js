...
},{"topojson-client":"../node_modules/topojson/node_modules/topojson-client/index.js","topojson-server":"../node_modules/topojson/node_modules/topojson-server/index.js","topojson-simplify":"../node_modules/topojson/node_modules/topojson-simplify/index.js"}],"data/missouri.topojson":[function(require,module,exports) {
module.exports = "/missouri.e0f2ac17.topojson";
},{}],"data/mo_birth_only_pols.csv":[function(require,module,exports) {
module.exports = "/mo_birth_only_pols.bdeee443.csv";
},{}],"map_edited.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#map_chart').append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var projection = d3.geoMercator();
var path = d3.geoPath().projection(projection);
var colorScale = d3.scaleOrdinal().range(['red', 'purple', 'blue']);
var div = d3.select('body').append('div').attr('class', 'tooltip').style('opacity', 0);
Promise.all([d3.json(require('./data/missouri.topojson')), d3.csv(require('./data/mo_birth_only_pols.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  var counties = topojson.feature(json, json.objects.missouri_export);
  projection.fitSize([width, height], counties); //console.log('Datapoints look like', datapoints)

  var cityList = ['St. Louis', 'Sikeston', 'Columbia', 'Independence', 'Jefferson City', 'Springfield', 'Houston'];
  var pronunciationList = ['Missour-ee', 'Both', 'Missour-ah'];
  var defs = svg.append('defs'); //this controls the outline (i.e., stroke) of the MO counties

  svg.selectAll('.counties').data(counties.features).enter().append('path').attr('class', 'counties').attr('d', path).attr('stroke', 'white').attr('fill', '#d3d3d3'); //this shows where the cities in the cityList are located in MO, for refernce

  svg.selectAll('.label').data(counties.features).enter().append('text').attr('class', function (d) {
  ...
  }).attr('text-anchor', 'right').attr('alignment-baseline', 'middle').style('visibility', 'hidden').attr('font-size', 3);
  defs.selectAll('.person-pattern').data(datapoints).enter().append('pattern').attr('class', 'artist-pattern').append('pattern').attr('id', function (d) {
  ...
  svg.selectAll('.circle').data(datapoints).enter().append('circle').attr('class', function (d) {
    var str = d['name_elected'].replace(/[\(\)]/g, ' ');
    return str.replace(/\s+/g, '-').toLowerCase();
  }).classed('pol', true).attr('transform', function (d) {
    var coords = projection([d.longitude, d.latitude]); //console.log(`(${coords})`)

    return "translate(".concat(coords, ")");
  }).attr('stroke', 'white').attr('stroke-width', 2).attr('fill', function (d) {
    return 'url(#' + d.name_elected.toLowerCase().replace(/ /g, '-') + ')';
  }).attr('visibility', 'visible').on('mouseover', function (d) {
    d3.select(this).raise().transition().duration(100).attr('r', 25).attr('stroke', function (d) {
      if (d['party'] === 'D') {
        return '#2f36c8';
      } else {
        return '#e9412b';
    ...
    div.html('<strong>' + 'Politician: ' + '</strong>' + d.name_elected + ' (' + d.party + '-MO)' + '<br>' + '<strong>' + 'Office: ' + '</strong>' + d.position + '<br>' + '<strong>' + 'Hometown: ' + '</strong>' + d.hometown + '<br>' + '<strong>' + 'Pronunciation: ' + '</strong>' + '"' + d.pronunciation + '"').style('left', d3.event.pageX + 20 + 'px').style('top', d3.event.pageY - 28 + 'px');
  }).on('mouseout', function (d) {
    d3.select(this).transition().duration(150).attr('r', 20).attr('stroke', 'white').attr('stroke-width', 3).style('opacity', 1);
    div.transition().duration(150).style('opacity', 0);
  });
  svg.append('filter').attr('id', 'desaturate').append('feColorMatrix').attr('type', 'matrix').attr('fill-opacity', 1).attr('brightness', '500%').attr('contrast', '20%').attr('values', '0.7 0.7 0.7 0 0 0.7 0.7 0.7 0 0 0.7 0.7 0.7 0 0 0 0 0 1 0');
  d3.select('#blankMO').on('stepin', function () {
    svg.selectAll('.pol').style('visibility', 'visible').attr('r', 20);
    svg.selectAll('.label').style('visibility', 'visible').transition().duration(200).attr('opacity', 1);
  }); //dems who say missouree (including those who say both)

  d3.select('#pol1').on('stepin', function () {
    svg.selectAll('.pol').style('visibility', function (d) {
      if (d['party'] === 'D') {
        return 'visible';
      } else {
        return 'hidden';
    ...
    svg.selectAll('.label').transition().duration(200).attr('opacity', 1);
  }); //dems who say missourah (including those who do both)

  d3.select('#pol2').on('stepin', function () {
    svg.selectAll('.pol').style('visibility', function (d) {
      if (d['party'] === 'D') {
        return 'visible';
      } else {
  ...
  d3.select('#pol3').on('stepin', function () {
    svg.selectAll('.pol').style('visibility', 'visible').attr('filter', function (d) {
  ...
  d3.select('#pol4').on('stepin', function () {
    svg.selectAll('.pol').style('visibility', function (d) {
      if (d['party'] === 'R') {
        return 'visible';
      } else {
  ...
  d3.select('#pol5').on('stepin', function () {
    svg.selectAll('.pol').style('visibility', function (d) {
      if (d['party'] === 'R') {
        return 'visible';
      } else {
  ...
  d3.select('#pol6').on('stepin', function () {
    svg.selectAll('.pol').style('visibility', function (d) {
    ...
    }).transition().duration(200).attr('opacity', 1).attr('filter', function (d) {
      if (d['pronunciation'] === 'Both') {
        return 'none';
      } else {
        return 'url(#desaturate)';
  ...
  function render() {
    ...
    var screenHeight = window.innerHeight; // let screenHeight = window.innerHeight
  ...
  window.addEventListener('resize', (0, _debounce.debounce)(render, 400));
  render();
}
},{"d3":"../node_modules/d3/index.js","debounce":"../node_modules/debounce/index.js","topojson":"../node_modules/topojson/index.js","./data/missouri.topojson":"data/missouri.topojson","./data/mo_birth_only_pols.csv":"data/mo_birth_only_pols.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","map_edited.js"], null)
//# sourceMappingURL=/map_edited.532ba166.map