...
},{"topojson-client":"../node_modules/topojson/node_modules/topojson-client/index.js","topojson-server":"../node_modules/topojson/node_modules/topojson-server/index.js","topojson-simplify":"../node_modules/topojson/node_modules/topojson-simplify/index.js"}],"data/br_mg.topojson":[function(require,module,exports) {
module.exports = "/br_mg.8722f8f1.topojson";
},{}],"data/br_dam_final.csv":[function(require,module,exports) {
module.exports = "/br_dam_final.64ea2da5.csv";
...
var path = d3.geoPath().projection(projection); // const xPositionScale = d3.scalePoint().range([0, width])
// var yPositionScale = d3
//   .scaleLinear()
//   .range([height, 0])

var colorScale = d3.scaleOrdinal().range(['red', 'orange', 'yellow', 'green', 'blue']);
var div = d3.select('body').append('div').attr('class', 'tooltip').style('opacity', 0);
Promise.all([d3.json(require('./data/br_mg.topojson')), d3.csv(require('./data/br_dam_final.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  var cities = topojson.feature(json, json.objects.municipios_2010);
  projection.fitSize([width, height], cities);
  console.log('Datapoints look like', datapoints);
  var cityList = ['Belo Horizonte', 'Uberlândia', 'Montes Claros', 'Brumadinho', 'Mariana', 'Governador Valadares'];
  svg.selectAll('.cities').data(cities.features).enter().append('path').attr('class', 'cities').attr('d', path) //changes the outline color of the counties
  .attr('stroke', 'white').attr('fill', '#d3d3d3');
  svg.selectAll('.label').data(cities.features).enter().append('text').attr('class', function (d) {
  ...
  svg.selectAll('.circle').data(datapoints).enter().append('circle').attr('class', function (d) {
    // console.log(d)
    var str = d['dam_name'].replace(/[\(\)]/g, ''); // console.log(str.replace(/\s+/g, '-').toLowerCase())

    return str.replace(/\s+/g, '-').toLowerCase();
  }).classed('dam', true).attr('transform', function (d) {
    var coords = projection([d.longitude, d.latitude]);
    return "translate(".concat(coords, ")rotate(-45)");
  }).attr('stroke-width', 4).attr('fill', function (d) {
    return colorScale(d.risk_category);
  }).attr('opacity', 0).attr('stroke', function (d) {
    return colorScale(d.risk_category);
  }).attr('visibility', 'hidden').on('mouseover', function (d) {
    // set up tooltip text
    div.transition().duration(200).style('opacity', 0.9);
    div.html('<strong>' + 'Dam Name: ' + '</strong>' + d.dam_name + '<br>' + '<strong>' + 'City: ' + '</strong>' + d.city + '<br>' + '<strong>' + 'Owned by: ' + '</strong>' + d.company_name).style('left', d3.event.pageX + 20 + 'px').style('top', d3.event.pageY - 28 + 'px');
  ...
  d3.select('#blankMO').on('stepin', function () {
    svg.selectAll('.dam').style('visibility', 'hidden');
    svg.selectAll('.label').style('visibility', 'visible').transition().duration(200).attr('opacity', 1);
  }); // showing the first politician

  d3.select('#pol1').on('stepin', function () {
    svg.selectAll('.dam').transition().duration(200).style('visibility', 'hidden').attr('opacity', 0);
    svg.select('.barragem-i-córrego-feijão').transition().duration(200).style('visibility', 'visible').attr('opacity', 0.9).attr('r', 3).attr('stroke', 'red');
    svg.selectAll('.label').transition().duration(200).attr('opacity', 0);
    svg.selectAll('.brumadinho').transition().duration(200).attr('opacity', 1);
  }); // showing the dam collapsed three years ago

  d3.select('#pol2').on('stepin', function () {
    svg.selectAll('.dam').transition().duration(200).attr('opacity', 0);
    svg.select('.fundão').style('visibility', 'visible').transition().duration(200).attr('opacity', 0.9).attr('r', 3).attr('stroke', 'red');
    svg.selectAll('.label').transition().duration(200).attr('opacity', 0);
    svg.selectAll('.mariana').transition().duration(200).attr('opacity', 1);
  }); // showing all the dam in MG on risks

  d3.select('#pol3').on('stepin', function () {
    svg.selectAll('.dam').style('visibility', 'visible').attr('opacity', function (d) {
  ...
  d3.select('#pol4').on('stepin', function () {
    ...
    svg.selectAll('.label').style('visibility', 'visible').transition().duration(200).attr('opacity', 0);
  }); // showing all the dam owned by Vale

  d3.select('#pol5').on('stepin', function () {
    svg.selectAll('.dam').style('visibility', 'visible').transition().duration(200).attr('opacity', function (d) {
  ...
  function render() {
    ...
    svg.selectAll('.cities').attr('d', path);
    svg.selectAll('.label').attr('transform', function (d) {
      return "translate(".concat(path.centroid(d), ")");
    });
    svg.selectAll('.dam').attr('transform', function (d) {
      var coords = projection([d.longitude, d.latitude]);
      return "translate(".concat(coords, ")rotate(-45)");
    }); // If it's really small, resize the circle size

    if (newHeight < 400) {
      svg.selectAll('.dam').attr('cx', 1).attr('cy', 1).attr('r', 1);
    } else {
      svg.selectAll('.dam').attr('cx', 3).attr('cy', 3).attr('r', 3);
    }
...
},{"d3":"../node_modules/d3/index.js","debounce":"../node_modules/debounce/index.js","topojson":"../node_modules/topojson/index.js","./data/br_mg.topojson":"data/br_mg.topojson","./data/br_dam_final.csv":"data/br_dam_final.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
